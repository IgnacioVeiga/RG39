...
- `RandomGameLauncher/Services`: infrastructure adapters (JSON repository, Steam/Epic providers, executable picker, launcher, dialog service).
...
2. `MainViewModel` loads manual games from `IGameRepository`.
3. Library providers (`Steam`, `Epic`) append discovered games.
